...
def normalize_points_to_unit_cube(points: np.ndarray) -> np.ndarray:
    """
    Normalize point cloud to [-1, 1]^3 cube.
...
def main(cfg) -> None:
    ...
    try:
        ...
        points = fps_coarse_surface[:, 0, :3]
        points_normalized = normalize_points_to_unit_cube(points)
        os.makedirs(os.path.join(cfg.output.save_dir), exist_ok=True)
        ply_output_path = os.path.join(cfg.output.save_dir, "sample_latent_norm_pc.ply")
        normal_path = os.path.join(cfg.output.save_dir, "norm_pc_normals.npz")
        save_ply_normal_open3d(points_normalized, ply_output_path, normal_path)
        return True
...
def save_ply_normal_open3d(vertices, filepath, normal_path):
    """Save point cloud vertices as PLY file using Open3D."""
    points = vertices
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(points)
    ...
    np.savez(normal_path, points=points, normals=normals)
